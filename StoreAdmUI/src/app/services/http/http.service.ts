...
export class HttpService{
  ...
  public get(url: string, options: HttpOptions = {}): Observable<APIResponse<any>> {
    const takingLong = this.setTakingLongTimeout();
    
    return this.httpClient.get<APIResponse<any>>(`${environment.apiURL}/${url}?${this.getQuery(options?.params)}`, {
  ...
  public post(url: string, data: any = null, options: HttpOptions = {}): Observable<APIResponse<any>> {
    const takingLong = this.setTakingLongTimeout();

    return this.httpClient.post<APIResponse<any>>(`${environment.apiURL}/${url}?${this.getQuery(options?.params)}`, data, {
      headers: this.getHeaders()
    }).pipe(
      timeout((this._takingLongSeconds / 2) * 3 * 1000),
      retryWhen(error => this.retryRequest(error)),
  ...
  public put(url: string, data: any = null, options: HttpOptions = {}): Observable<APIResponse<any>> {
    const takingLong = this.setTakingLongTimeout();

    return this.httpClient.put<APIResponse<any>>(`${environment.apiURL}/${url}?${this.getQuery(options?.params)}`, data, {
      headers: this.getHeaders()
    }).pipe(
      timeout((this._takingLongSeconds / 2) * 3 * 1000),
      retryWhen(error => this.retryRequest(error)),
  ...
  public delete(url: string, options: HttpOptions = {}): Observable<APIResponse<any>> {
    const takingLong = this.setTakingLongTimeout();

    return this.httpClient.delete<APIResponse<any>>(`${environment.apiURL}/${url}?${this.getQuery(options?.params)}`, {
      headers: this.getHeaders()
    }).pipe(
      timeout((this._takingLongSeconds / 2) * 3 * 1000),
      retryWhen(error => this.retryRequest(error)),
  ...
  private getQueryParam(prop: any, param: unknown, prefix?: any): any {
    prop = prefix != null ? `${prefix}.${prop}` : prop;

    if(param == null){
      return `${prop}=&`
    } else if(param instanceof Date) {
      return `${prop}=${param.toDateString()}&`;
    } else {
      return `${prop}=${param}&`
    } 
  }
